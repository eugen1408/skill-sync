<script lang="ts">
  ...
  const { title, body, size = 14 }: Props = $props()
</script>
...
<details class="relative inline-block align-middle">
  ...
  <div
    class="absolute left-0 z-50 mt-1 w-72 rounded border border-surface-200-800 bg-surface-50-950 p-3 text-left text-xs shadow-lg"
  >
    {#if title}

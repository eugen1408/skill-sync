...
const en = {
  ...
  'sources.confirmRemoveDetail': 'All installed skills from this source will remain, but will no longer receive updates.',
  ...
  'settings.confirmDeleteTokenDetail': 'You will no longer be able to use the token for GitHub API requests.',
...
const ru: Record<keyof typeof en, string> = {
  ...
  'sources.confirmRemoveDetail': 'Установленные из него скиллы останутся, но перестанут обновляться.',
  ...
  'settings.confirmDeleteTokenDetail': 'Приложение больше не сможет использовать его для запросов к GitHub API.',
  'settings.secretsUnavailable':
